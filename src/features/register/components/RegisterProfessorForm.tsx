...
type RegisterProfessorFormBody = {
  name: string;
  idNumber: string;
  email: string;
  password: string;
...
const RegisterProfessorForm = () => {
  const [isLoading, setIsLoading] = useState(false);
  const { control, handleSubmit, setError, getValues } =
    useForm<RegisterProfessorFormBody>({
      mode: "onChange",
    });

  const navigation = useUnauthNavigation();

  const onPressSubmit = async (data: RegisterProfessorFormBody) => {
    setIsLoading(true);
    try {
      const reqBody = {
        email: data.email,
...
export default RegisterProfessorForm;
